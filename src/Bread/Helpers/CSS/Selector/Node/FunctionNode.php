 ...
 */
namespace Bread\Helpers\CSS\Selector\Node;

use Bread\Helpers\CSS\Selector\Interfaces;
use Bread\Helpers\CSS\Selector\XPathExpr;
...
class FunctionNode implements Interfaces\Node
{
    ...
    protected function _xpath_nth_child($xpath, $expr, $last = false, $addNameTest = true)
    {
        ...
        return $xpath;
        /*
         * FIXME: handle an+b, odd, even an+b means every-a, plus b, e.g., 2n+1 means odd 0n+b means b n+0 means a=1,
         * i.e., all elements an means every a elements, i.e., 2n means even -n means -1n -1n+6 means elements 6 and
         * previous
         */
    }
    ...
    protected function _xpath_contains($xpath, $expr)
    {
        ...
        // $xpath->addCondition(sprintf('contains(lower-case(string(.)), %s)',
        // XPathExpr::xpathLiteral(strtolower($expr))));
